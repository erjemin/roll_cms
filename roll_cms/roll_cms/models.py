...
class TbRoll(models.Model):
    ...
    szRollOldSlugs = models.JSONField(
        ...
        help_text="JSON-массив строк (типа <b>[\"старый_слаг_1\", \"старый_слаг_2\", \"и так далее\"б ]</b>) из"
                  ...
                  " редирект будет производиться при обработке ошибки 404 (страница не найдена) и поэтому будет очень"
                  " медленным и порождать лишнюю нагрузку... Редирект будет производится с кодом 301"
                  " (постоянный редирект)</small>"
    )
